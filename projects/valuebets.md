...
- Basketball (National Basketball Association): 9 seasons of data were collected, starting from the 2014-2015 season. The 2022-2023 season was set aside as a holdout dataset for testing model performance.
...
In terms of the method, my plan is to train first look at whether the historical win rate conditional on the odds favoring the team. Then I'll look at a random forest model to see if it can identify better value bets that result in a performance for betting. Then I'll train two NN models. First a very simple feedforward model with 3 hidden layers and 1 output layer on only basketball filtered data. Second a NCF model with 3 hidden layers and 1 output layer trained on all sports data -> (chat gpt you have to provide a rationale about where the CF part comes into play).


In terms of evaluation betting performance, I will be evaluating which model is better by assessing each model's value bet EV and ROI for both the home and away team. I'm looking at EV because it gives us a sense of for each marginal/incremental bet, what we can expect and then ROI to look at the returns over time. I'll also compare each model's value bet EV and ROI to straight-up betting the home team and away team to evaluate the models against a dummy baseline betting strategy.


I'm going to split the data as follows. 80% test, 10% validation, and then 10% test. I'm going to set aside the nba season 2022-2023 as a separate holdout test data. 
